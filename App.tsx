...
import { Tool, CanvasElement, Point, ToolOptions, PenElement, ShapeElement, ImageElement, TextElement, AIState, LineElement, ArrowElement, PanOffset } from './types';
...
// --- Utility Functions ---
const getClientCoords = (event: React.MouseEvent | React.TouchEvent | MouseEvent | TouchEvent): Point => {
    if ('touches' in event) {
...
const getCanvasCoords = (event: React.MouseEvent | React.TouchEvent | MouseEvent | TouchEvent, panOffset: PanOffset): Point => {
...
const isPointInsideElement = (point: Point, element: CanvasElement): boolean => {
        ...
        switch (element.tool) {
            ...
            case Tool.PEN:
                const onStroke = element.points.some((p, index) => {
                    ...
                    return distanceToLineSegment(point, p, nextP) < element.strokeWidth / 2 + 5; // Tolerance
                });
                if (onStroke) return element;
                break;
            case Tool.LINE:
            case Tool.ARROW:
                const dist = distanceToLineSegment(point, element.points[0], element.points[1]);
                if (dist < element.strokeWidth / 2 + 5) { // Tolerance
                    return element;
                }
                break;
            default:
...
const createElement = (id: number, x: number, y: number, tool: Tool, options: ToolOptions): CanvasElement => {
    const { strokeColor, strokeWidth, strokeStyle, opacity, backgroundColor } = options;
    const baseElement = { id, x, y, width: 0, height: 0, strokeColor, strokeWidth, strokeStyle, opacity };

    switch (tool) {
        case Tool.RECTANGLE: {
            const element: ShapeElement = { ...baseElement, tool: Tool.RECTANGLE, backgroundColor, fillStyle: 'solid' };
            return element;
        }
        case Tool.CIRCLE: {
            const element: ShapeElement = { ...baseElement, tool: Tool.CIRCLE, backgroundColor, fillStyle: 'solid' };
            return element;
        }
        case Tool.TEXT: {
            const element: TextElement = { ...baseElement, tool: Tool.TEXT, text: 'Text', fontSize: 24, fontFamily: 'Inter' };
            return element;
        }
        case Tool.LINE: {
            const element: LineElement = { ...baseElement, tool: Tool.LINE, points: [{x,y}, {x,y}] };
            return element;
        }
        case Tool.ARROW: {
            const element: ArrowElement = { ...baseElement, tool: Tool.ARROW, points: [{x,y}, {x,y}] };
            return element;
        }
        case Tool.PEN:
        default: {
            const element: PenElement = { ...baseElement, tool: Tool.PEN, points: [{ x, y }] };
            return element;
        }
    }
};
...
const drawElement = (context: CanvasRenderingContext2D, element: CanvasElement) => {
    ...
    switch (element.tool) {
        ...
        case Tool.ARROW:
            ...
            const p1 = { x: start.x + thickness/2 * Math.cos(angle90), y: start.y + thickness/2 * Math.sin(angle90) };
            const p2 = { x: start.x - thickness/2 * Math.cos(angle90), y: start.y - thickness/2 * Math.sin(angle90) };
            const p3 = { x: bodyEndPoint.x - thickness/2 * Math.cos(angle90), y: bodyEndPoint.y - thickness/2 * Math.sin(angle90) };
            const p4 = { x: bodyEndPoint.x + thickness/2 * Math.cos(angle90), y: bodyEndPoint.y + thickness/2 * Math.sin(angle90) };

            const headP1 = end; // The tip of the arrow
            const headP2 = { x: bodyEndPoint.x - arrowHeadWidth/2 * Math.cos(angle90), y: bodyEndPoint.y - arrowHeadWidth/2 * Math.sin(angle90) };
            const headP3 = { x: bodyEndPoint.x + arrowHeadWidth/2 * Math.cos(angle90), y: bodyEndPoint.y + arrowHeadWidth/2 * Math.sin(angle90) };
        ...
        case Tool.CIRCLE:
            context.beginPath();
            context.arc(element.x + element.width / 2, element.y + element.height / 2, Math.max(Math.abs(element.width), Math.abs(element.height)) / 2, 0, 2 * Math.PI);
            context.stroke();
            break;
        case Tool.TEXT:
            context.fillStyle = element.strokeColor;
...
const App: React.FC = () => {
  ...
  const [action, setAction] = useState<'none' | 'drawing' | 'moving' | 'panning' | 'writing' | 'erasing'>('none');
  const [tool, setTool] = useState<Tool>(Tool.HAND);
  const [options, setOptions] = useState<ToolOptions>({ strokeColor: '#ffffff', backgroundColor: '#ffffff', strokeWidth: 4, strokeStyle: 'solid', opacity: 1 });
  ...
  useLayoutEffect(() => {
    ...
    elements.forEach(element => {
      ...
      if (selectedElement && selectedElement.id === element.id && action !== 'writing') {
        const { x1, y1, x2, y2 } = getNormalizedPosition(element);
        context.save();
    ...
    if (laserTrail.length > 0) {
        laserTrail.forEach((point, index) => {
            ...
            context.beginPath();
            context.arc(point.x, point.y, 8 * (index + 1) / laserTrail.length, 0, 2 * Math.PI);
            context.fill();
        });
    }
  ...
  const updateElement = (element: CanvasElement, updates: Partial<CanvasElement>) => {
    const elementsCopy = [...elements];
    const index = elementsCopy.findIndex(el => el.id === element.id);
    if (index === -1) return;

    elementsCopy[index] = { ...elementsCopy[index], ...updates };
    setElements(elementsCopy);
    if(action !== 'writing') updateHistory(elementsCopy);
  };
  
   const updateHistory = useCallback((newElements: CanvasElement[]) => {
  ...
  const handleMouseDown = (event: React.MouseEvent) => {
    ...
    if (tool === Tool.HAND) {
        ...
        const element = getElementAtPosition(canvasCoords.x, canvasCoords.y, elements);
        setSelectedElement(element);
        if (element) {
            setAction('moving');
            setMovingOffset({ x: canvasCoords.x - element.x, y: canvasCoords.y - element.y });
        ...
        const id = Date.now();
        const newElement = createElement(id, canvasCoords.x, canvasCoords.y, tool, options);
        if(newElement.tool === Tool.ARROW) {
            const startBinding = getElementAtPosition(canvasCoords.x, canvasCoords.y, elements);
            if(startBinding) (newElement as ArrowElement).startBinding = startBinding.id;
        }
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    ...
    else if (tool === Tool.SELECTION) (event.target as HTMLElement).style.cursor = getElementAtPosition(canvasCoords.x, canvasCoords.y, elements) ? 'move' : 'default';
    else if (tool === Tool.LASER) (event.target as HTMLElement).style.cursor = 'none';
    ...
    if (action === 'panning') {
        ...
        switch (currentElement.tool) {
            ...
            case Tool.LINE:
            case Tool.ARROW:
                 (currentElement as LineElement | ArrowElement).points[1] = canvasCoords;
                 break;
        }
        ...
        setElements(prevElements => {
            ...
            if (updatedElement.tool === Tool.PEN) {
                updatedElement.points = updatedElement.points.map(p => ({ x: p.x + dx, y: p.y + dy }));
            }
            ...
            return elementsCopy.map(el => {
                if (el.tool === Tool.ARROW) {
                    ...
                    if (newArrow.startBinding === updatedElement.id) {
                        newArrow.points = [getElementCenter(updatedElement), newArrow.points[1]];
                        updated = true;
                    }
                    if (newArrow.endBinding === updatedElement.id) {
                        newArrow.points = [newArrow.points[0], getElementCenter(updatedElement)];
                        updated = true;
                    }
        ...
        const elementToErase = getElementAtPosition(canvasCoords.x, canvasCoords.y, elements);
        if (elementToErase) {
    ...
    if(selectedElement && action === 'drawing') {
        ...
        if (index !== -1) {
            ...
            if(currentElement.tool === Tool.ARROW) {
                const endBinding = getElementAtPosition(canvasCoords.x, canvasCoords.y, elements);
                if (endBinding) {
      ...
      if (newText.trim() === '') {
          ...
          if (index !== -1) {
              ...
              if (elementToUpdate.tool === Tool.TEXT) {
                  const textWidth = textAreaRef.current?.scrollWidth || elementToUpdate.width;
                  const textHeight = textAreaRef.current?.scrollHeight || elementToUpdate.height;
            
                  elementsCopy[index] = { ...elementToUpdate, text: newText, width: textWidth, height: textHeight };
                  setElements(elementsCopy);
  ...
  const handleAddDiagram = (svgDataUrl: string) => {
    ...
    image.onload = () => {
      ...
      const newImageElement: ImageElement = {
        id, tool: Tool.DIAGRAM,
        x: 100 - panOffset.x, y: 100 - panOffset.y,
        width: 400, height: 400 / aspectRatio,
        base64: svgDataUrl, image,
        strokeColor: '', strokeWidth: 0, strokeStyle: 'solid', opacity: 1,
      };
  ...
  return (
    <div className="w-full h-full relative font-sans text-white overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-10 flex items-center space-x-2">
        <button onClick={exportCanvas} className="bg-[#2D2D2D] text-gray-200 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#3a3a3a] transition-colors">Compartilhar</button>
      </div>
      ...
      <div className="absolute bottom-4 left-4 z-10 flex items-center space-x-2">
          <div className="flex items-center bg-[#252525] p-1 rounded-lg shadow-xl">
             <button onClick={undo} disabled={historyIndex < 0} className="p-2 text-gray-300 hover:bg-[#3a3a3a] rounded-md disabled:opacity-50 disabled:cursor-not-allowed"><UndoIcon className="w-5 h-5"/></button>
             <button onClick={redo} disabled={historyIndex >= history.length - 1} className="p-2 text-gray-300 hover:bg-[#3a3a3a] rounded-md disabled:opacity-50 disabled:cursor-not-allowed"><RedoIcon className="w-5 h-5"/></button>
             <div className="w-px h-6 bg-gray-600 mx-1"></div>
             <button onClick={clearCanvas} className="p-2 text-red-400 hover:bg-red-900/50 rounded-md"><TrashIcon className="w-5 h-5"/></button>
          </div>
      </div>
      ...
      {!hasStarted && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 pointer-events-none z-0">
            <div className="flex flex-col items-center p-8 rounded-2xl">
                <svg className="w-16 h-16 text-violet-400 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path></svg>
                <h1 className="text-3xl md:text-5xl font-bold text-gray-100 mb-2">Quadro Branco</h1>
                <p className="text-lg md:text-xl text-gray-400">Desenhe, crie e colabore. Todos os dados são salvos localmente.</p>
            </div>
        </div>
      )}

      {action === 'writing' && selectedElement && selectedElement.tool === Tool.TEXT && (
        <textarea
            ref={textAreaRef}
            ...
            style={{
                ...
                width: getNormalizedPosition(selectedElement).x2 - getNormalizedPosition(selectedElement).x1,
                height: getNormalizedPosition(selectedElement).y2 - getNormalizedPosition(selectedElement).y1,
